...
func getServiceLogs() {
	...
	cluster := config.EcsherConfigManager.GetCluster(logsOptions.Cluster, RootOptions.profile)
	client := ecs.GetClient(logsOptions.Region, RootOptions.profile)
...
func getTaskLogs() {
	...
	cluster := config.EcsherConfigManager.GetCluster(logsOptions.Cluster, RootOptions.profile)
	ecsClient := ecs.GetClient(logsOptions.Region, RootOptions.profile)
